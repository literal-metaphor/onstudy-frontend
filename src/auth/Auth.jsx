        ...
        <MouseParallaxContainer className="tw-h-full" globalFactorX={0.1} globalFactorY={0.1}>
          <div className="row gx-4 tw-h-full">
            ...
            <main className="tw-h-full col-12 col-lg-8 p-5 d-flex flex-column justify-content-center align-items-center lg:tw-rounded-l-3xl tw-bg-[#fff]">
              <div className="tw-relative w-100 h-100 d-flex justify-content-center align-items-center">
              {/* Sign Up */}
              <form className={`w-100 px-4 tw-absolute tw-transition tw-duration-300 ${page === "SignUp" && "tw-translate-x-[100vw]"}`}>
                ...
                <div className="mb-3 form-check">
                  ...
                  <label className="form-check-label hover:tw-cursor-pointer"><span onClick={()=>setAgreed(!agreed)}>Saya menyutujui</span> <span className="text-primary" onClick={()=>document.getElementById('tos_modal').showModal()}>Persyaratan Layanan onStudy</span></label>
                </div>
                <dialog id="tos_modal" className="tw-modal">
                ...
                <button disabled={agreed} type="submit" className="btn btn-success w-100 mt-4 py-2 px-4 rounded tw-font-normal tw-text-white roboto tw-transition tw-duration-300">Daftar</button>
              </form>
              {/* Login */}
              <form className={`w-100 px-4 tw-absolute text-center tw-transition tw-duration-300 ${page === "Login" && "tw-translate-x-[100vw]"}`}>
                ...
                <button type="submit" className="btn btn-success w-100 my-4 py-2 px-4 rounded tw-font-normal tw-text-white roboto tw-transition tw-duration-300">Masuk</button>
                <a className="text-primary" href="#">Lupa Password?</a>
              </form>
              </div>
              {/* Divider */}
              <div className="tw-divider py-4">Atau</div>
              {/* Third-Party */}
              ...
              {/* Change Page */}
              {page === "Login" && (
                <p>Belum Punya Akun? <span className="hover:tw-cursor-pointer text-primary" onClick={() => setPage("SignUp")}>Daftar</span></p>
              )}
              {page === "SignUp" && (
                <p>Sudah Punya Akun? <span className="hover:tw-cursor-pointer text-primary" onClick={() => setPage("Login")}>Masuk</span></p>
              )}
            </main>
          </div>
